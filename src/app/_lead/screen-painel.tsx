...
export function ScreenPainel({ s, act }: { s: FlowState; act: FlowActions }) {
  ...
  return (
    <main id="conteudo" className="flex flex-1 px-6 py-2">
      <Card pad="sm" className="m-auto flex w-full max-w-md flex-col gap-3">
        <BrandDots size="sm" />
        ...
        <BackgroundGradient containerClassName="mx-auto w-full max-w-[240px]" className="rounded-3xl bg-white/85 px-3 pb-3 pt-2.5 backdrop-blur-md">
          <p className="mb-1.5 text-center text-[10px] font-extrabold uppercase tracking-[0.15em] text-brand-blue/80">
            Seu futuro diploma
          </p>
          <div className="flex w-full flex-col items-center">
            <Parchment rollerClassName="h-[11px]" bodyClassName="px-4 pb-4 pt-3.5">
              <div className="flex flex-col items-center gap-1.5">
                ...
                <ParchmentPortrait name={s.name} photo={null} size={54} />
                <p className="text-center font-serif text-[15px] font-extrabold leading-tight tracking-[0.01em] text-[#3f2f12]">
                  {s.name.toUpperCase()}
                </p>
                <div aria-hidden className="h-px w-12 bg-[rgba(120,90,30,0.4)]" />
                <p className="text-center font-serif text-[12px] leading-[1.4] text-[#5b4a24]">
                  Certificado de conclusão
                  <br />
                  <strong className="text-[#3f2f12]">reservado em seu nome</strong>
                </p>
                <span className="inline-flex items-center gap-1.5 rounded-full border border-[rgba(166,124,0,0.45)] bg-[rgba(255,223,0,0.18)] px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.07em] text-[#8a6a00]">
        ...
        {s.stage === "lead" && (
          <div className="flex flex-col gap-2.5">
            {!semCheckout && (
            <div className="flex items-center justify-between gap-3 rounded-2xl border border-brand-border bg-white/75 px-4 py-2.5">
            ...
            <button
              ...
              className={`${styles.shiny} flex min-h-[54px] cursor-pointer items-center justify-center gap-2 rounded-xl border-none bg-brand-green-dark px-5 text-[19px] font-extrabold tracking-tight text-white shadow-[0_12px_30px_-12px_rgba(0,156,59,0.6)]`}
            >
              Quero mudar de vida →
            </button>
        ...
        <button
          type="button"
          onClick={act.logout}
          className="min-h-8 cursor-pointer self-center border-none bg-transparent text-[13px] font-semibold text-brand-muted underline underline-offset-4"
        >
          Sair da conta
        </button>
      </Card>
