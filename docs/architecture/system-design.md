...
flowchart TD
    ...
    subgraph Data_Storage["💾 Persistência & Cache"]
        Postgres[("PostgreSQL 16<br>dbs: backend, notify, evolution")]
    ...
    subgraph External_Integrations["🔌 Provedores & Gateways"]
        ...
        SMTP["Mailcow / SMTP<br>(E-mails Transacionais)"]
    end
...
2. **Isolamento de Bancos de Dados**:
   - O container PostgreSQL hospeda bancos logicamente separados:
     - `backend`: Usuários, leads, candidatos, matrículas, pagamentos e polos.
     - `notify`: Contas, templates, disparos, logs de envio e supressões.
     - `evogo_auth` / `evogo_users`: Sessões de conexão WhatsApp do Evolution Go.
