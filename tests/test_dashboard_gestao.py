...
def _mk(account, **kw):
    defaults = dict(
        ...
        whatsapp_status="skipped", email_status="skipped", tts_status="skipped",
    )
    defaults.update(kw)
...
def test_filtro_canal_status_e_provider(client, account):
    _mk(account, want_whatsapp=True, whatsapp_status="failed", driver_used="evolution-v2")
...
def test_status_lista_os_quatro_servicos(client, monkeypatch):
    ...
    for nome in ("Evolution v2", "Evolution GO", "mailcow", "OmniRouter"):
        assert nome in body
