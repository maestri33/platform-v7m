...
def evolutions_ok(monkeypatch):
    """Ambas as Evolutions respondendo; registra o que foi chamado."""
    chamadas: dict[str, list] = {"v2": [], "go": [], "webhook": []}

    def v2_ensure(*, instance_name, phone_number=""):
        chamadas["v2"].append(instance_name)
        return {"instanceName": instance_name}, True

    def go_ensure(*, instance_name):
        chamadas["go"].append(instance_name)
        return {"name": instance_name, "token": f"tok-{instance_name}"}, True

    monkeypatch.setattr("whatsapp.provisioning.v2_ensure_instance", v2_ensure)
    monkeypatch.setattr("whatsapp.provisioning.go_ensure_instance", go_ensure)
    monkeypatch.setattr(
        "whatsapp.provisioning.v2_set_webhook",
        lambda name, url=None: chamadas["webhook"].append(("v2", name)),
    )
    monkeypatch.setattr(
...
def mailcow_ok(monkeypatch):
    ...
    monkeypatch.setattr("mail.mailcow.get_client", lambda: _MC())
    yield
...
def test_provisiona_app_completo(evolutions_ok, mailcow_ok):
    ...
    voices = account.tts_voices.get()
    assert voices.voice_male and voices.voice_female


def test_mesmo_numero_vira_instancia_nos_dois_evolutions(evolutions_ok, mailcow_ok):
    provision_app(slug="dois", phone_number="554288887777", seed_templates=False)
    assert evolutions_ok["v2"] == ["dois"]
    assert evolutions_ok["go"] == ["dois"]
    assert ("v2", "dois") in evolutions_ok["webhook"]
    assert ("go", "dois") in evolutions_ok["webhook"]
...
def test_reprovisionar_nao_duplica_numero_nem_identidade(evolutions_ok, mailcow_ok):
    ...
    assert TtsVoices.objects.filter(account=account).count() == 1
...
def test_mailcow_fora_nao_impede_conta_e_whatsapp(evolutions_ok, monkeypatch):
    def explode():
        raise RuntimeError("mailcow inacessível")

    monkeypatch.setattr("mail.mailcow.get_client", explode)
...
def test_evolution_v2_fora_ainda_deixa_a_cadeia_registrada(monkeypatch, mailcow_ok):
    def v2_explode(*, instance_name, phone_number=""):
        raise RuntimeError("v2 fora do ar")

    monkeypatch.setattr("whatsapp.provisioning.v2_ensure_instance", v2_explode)
    monkeypatch.setattr(
        "whatsapp.provisioning.go_ensure_instance",
        lambda *, instance_name: ({"name": instance_name, "token": "t"}, True),
    )
    monkeypatch.setattr("whatsapp.provisioning.go_set_webhook", lambda *a, **k: None)

    report = provision_app(slug="sov2", seed_templates=False)
    status = {s.name: s.status for s in report.steps}

    assert status["evolution_v2"] == FAILED
    assert status["evolution_go"] == OK
    assert status["whatsapp_number"] == OK
    numero = WhatsAppNumber.objects.get(account__slug="sov2")
...
def test_dominio_inexistente_no_mailcow_falha_cedo(evolutions_ok, monkeypatch):
    ...
    assert "não existe no mailcow" in detalhe
...
def test_endpoint_devolve_207_quando_um_passo_falha(
    ...
    monkeypatch.setattr(
        "mail.mailcow.get_client", lambda: (_ for _ in ()).throw(RuntimeError("fora"))
    )
