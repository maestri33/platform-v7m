...
def test_notifications_expoe_campos_novos(client, auth_headers):
    ...
    for k in ("external_id", "caller", "recipient_phone", "recipient_email",
              "whatsapp_status", "email_status", "tts_status", "attempts", "created_at"):
        assert k in item
    # campos novos
...
def test_filtros_por_status(client, auth_headers):
    ...
    # ambos sem canal email → skipped; tts idem
    resp = client.get("/v1/notifications?email_status=skipped", headers=auth_headers)
    assert len(resp.json()) == 2
    resp = client.get("/v1/notifications?tts_status=sent", headers=auth_headers)
    assert resp.json() == []
...
def test_is_tts_override_liga(client, auth_headers, account):
    _template(account, is_tts=False)
    resp = _send_event(client, auth_headers, is_tts_override=True, gender="M")
    n = Notification.objects.get(external_id=resp.json()["external_id"])
    assert n.want_tts is True
    assert n.gender == "M"


def test_is_tts_override_desliga(client, auth_headers, account):
    _template(account, is_tts=True)
