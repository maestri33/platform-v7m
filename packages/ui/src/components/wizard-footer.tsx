...
export function WizardFooter({ buttons }: WizardFooterProps) {
  ...
  return (
    <footer className="z-30 shrink-0 border-t border-white/10 bg-brand-ink/40 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-md items-center justify-center gap-3 px-6 py-3">
        {buttons.map((b, i) => (
          <Button
            key={b.label}
            ...
            disabled={b.disabled}
            onClick={b.onClick}
            className="flex-1 min-h-12 text-base font-extrabold shadow-md"
          >
            {b.label}
          </Button>
        ))}
