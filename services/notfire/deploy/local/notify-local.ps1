...
function Test-ComposeDefinition {
    ...
    $expectedImages = @{
        'evolution-v2' = 'evoapicloud/evolution-api:v2.3.7'
...
function Wait-Healthy {
    $required = @('postgres', 'redis', 'evolution-v2', 'evolution-go', 'notify-web', 'notify-worker')
    ...
    do {
        ...
        if ($pending.Count -eq 0) {
            Write-Host '[ok] Todos os seis servicos estao healthy.' -ForegroundColor Green
            return
        }
...
try {
    switch ($Action) {
        'setup' {
            ...
            Invoke-Docker pull postgres redis evolution-v2 evolution-go
        ...
        'diagnose' {
            Invoke-Docker ps --all
            Invoke-Docker logs --tail 80 postgres redis evolution-v2 evolution-go notify-web notify-worker
        }
        'verify' {
            Test-ComposeDefinition
            Test-RunningStack
