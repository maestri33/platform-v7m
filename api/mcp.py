...
TOOLS = [
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {
                "text": {"type": "string", "description": "Corpo da mensagem (markdown simples)"},
                "phone": {"type": "string", "description": "E.164 sem '+', ex.: 5542999999999"},
                "email": {"type": "string"},
                "subject": {"type": "string", "description": "Assunto do e-mail"},
                "title": {"type": "string"},
                "tts": {"type": "boolean", "description": "Entregar como nota de voz no WhatsApp"},
                "gender": {"type": "string", "enum": ["M", "F"], "description": "Seleção de voz do TTS"},
                "media_url": {"type": "string"},
                "media_type": {"type": "string", "enum": ["image", "video", "audio", "document"]},
                "external_id": {"type": "string", "description": "Chave de idempotência"},
                "caller": {"type": "string", "description": "Rótulo de origem para auditoria"},
            },
            "required": ["text"],
        },
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "ctx": {"type": "object", "description": "Valores dos placeholders {chave}"},
    ...
    },
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {"external_id": {"type": "string", "description": "external_id ou a chave de idempotência"}},
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "title": {"type": "string"},
                "subject": {"type": "string"},
                "channels": {"type": "string", "description": "csv: whatsapp,email"},
                "is_tts": {"type": "boolean"},
            },
            "required": ["event", "body_md"],
        },
    },
]
...
def _notif_dict(n) -> dict:
    return {
        ...
        "attempts": n.attempts,
        "error": n.whatsapp_error or n.email_error or n.tts_error,
...
def _tool_history(account, args: dict) -> dict:
    ...
    if args.get("status"):
        s = args["status"]
        qs = qs.filter(Q(whatsapp_status=s) | Q(email_status=s) | Q(tts_status=s))
...
def _tool_channels(account, _args: dict) -> dict:
    from channels.models import AppWebhook, MailIdentity, TtsVoices, WhatsAppNumber
    ...
    return {
        ...
        "tts": [
            {"voice_for_M": v.voice_male, "voice_for_F": v.voice_female}
            for v in TtsVoices.objects.filter(account=account)
        ],
