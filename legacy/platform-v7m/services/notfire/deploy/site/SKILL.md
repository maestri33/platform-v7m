...
- Stack: Django 5.1 + django-ninja, django-q2 (fila em DB), Postgres, Evolution v2 +
  Evolution GO (WhatsApp), OmniRouter→MiniMax (TTS e assistente de texto),
  SMTP/mailcow (e-mail).
...
|---|---|---|
| `WhatsAppNumber` | O **mesmo número** com instância na Evolution v2 (base) e na GO (fallback e funções extras). `driver` = preferido, `fallback_driver` = para onde cair. | painel → aba *whatsapp* |
| `MailIdentity` | Remetente SMTP (caixa do mailcow). Sem ela, o canal de e-mail fica indisponível. | painel → aba *e-mail* |
...
| POST | `/v1/send` | Envio direto (texto / mídia / nota de voz) |
...
Uma chamada deixa o app pronto: `Account` + `ApiKey`, instância com o **mesmo
número** na v2 **e** na GO (com webhook apontando de volta), caixa no mailcow +
...
{ "event": "status", "data": { "external_id": "...", "whatsapp_status": "sent",
  "delivery_status": "read", "driver_used": "evolution-v2", "attempts": 1 } }
```
