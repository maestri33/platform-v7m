...
export function AdminNav() {
  ...
  const items = isMasterPath
    ...
    ? HUB_ITEMS
    : activeContext === "promoter" || pathname.startsWith("/promoter") || pathname.startsWith("/vendas") || pathname.startsWith("/onboarding") || pathname.startsWith("/conta")
    ? PROMOTER_ITEMS
    : ADMIN_ITEMS;

  function isActive(href: string): boolean {
    if (href === "/dashboard" || href === "/hub" || href === "/promoter" || href === "/vendas") {
  ...
  return (
    <nav aria-label="Seções" className="shrink-0 space-y-3 hidden sm:block">
      ...
      <ul className="flex gap-1 overflow-x-auto pb-1 sm:sticky sm:top-20 sm:flex-col sm:gap-0.5 sm:overflow-visible sm:pb-0">
          ...
          return (
            <li key={it.href} className="shrink-0">
              <Link
                ...
                className={`flex items-center gap-2.5 whitespace-nowrap rounded-xl px-3.5 py-2.5 text-[14px] font-bold transition ${
                  active
        ...
        <li className="shrink-0 sm:mt-2 sm:border-t sm:border-brand-border sm:pt-2">
          <button
            type="button"
            onClick={logout}
            className="flex w-full items-center gap-2.5 whitespace-nowrap rounded-xl px-3.5 py-2.5 text-[14px] font-bold text-brand-muted transition hover:bg-brand-danger-bg hover:text-brand-danger"
          >
            <svg
              aria-hidden
              className="size-[18px] shrink-0"
...
export function AdminLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-2 px-4 py-4 pb-20 sm:flex-row sm:gap-6 sm:px-6 sm:py-6 sm:pb-6">
