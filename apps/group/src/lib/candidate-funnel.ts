import type { CandidateMe, CandidateStatus } from "@/lib/api-collaborators";
...
export const STAGE_HREF: Record<CandidateStatus, string> = {
  started: "/onboarding/documento",
  profile: "/onboarding/documento",
  address: "/onboarding/documento",
  documents: "/onboarding/pix",
  pix: "/onboarding/pix",
  education: "/onboarding/selfie",
...
/**
 * `STAGE_HREF` à prova de drift: se o backend devolver status inesperado,
 * cai em `/vendas` de forma segura.
 */
export function stageHref(status: string | undefined | null): string {
  return STAGE_HREF[status as CandidateStatus] ?? "/vendas";
}
...
export function candidateStageHref(me: CandidateMe): string {
  if (me.address_proof?.status === "rejected" || me.address_proof?.needs_kinship) {
    return "/onboarding/endereco";
  }
  const recovery = me.blocks?.find((block) =>
    ["rg", "cnh", "address_proof", "selfie"].includes(block.source_type)
  );
  if (recovery?.source_type === "address_proof") return "/onboarding/endereco";
  if (recovery?.source_type === "rg" || recovery?.source_type === "cnh") {
    return "/onboarding/documento";
  }
  if (recovery?.source_type === "selfie") return "/onboarding/selfie";
  if (["started", "profile", "address", "documents", "pix"].includes(me.status)) {
    if (!hasCapturedIdentityDocument(me)) return "/onboarding/documento";
    if (!me.address_proof?.photo) return "/onboarding/endereco";
    if (!me.pix_validated) return "/onboarding/pix";
    return "/onboarding/escolaridade";
  }
...
/** Próximo passo depois de concluir cada etapa (navegação direta dos formulários). */
export const NEXT_STAGE: Record<string, string> = {
  profile: "/onboarding/documento",
  address: "/onboarding/pix",
  documents: "/onboarding/endereco",
  pix: "/onboarding/escolaridade",
  education: "/onboarding/selfie",
  selfie: "/vendas",
};

/**
 * A etapa `stage` já foi concluída pra quem está em `current`?
 */
...
/**
 * Trata resposta 409 (WRONG_STATUS) e resolve o destino adequado.
 */
