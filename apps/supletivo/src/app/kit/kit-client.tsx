...
export function KitClient() {
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-8 px-4 py-6">
      ...
      {/* ── background gradient ───────────────────────────────────── */}
      <Section title="🌈 BackgroundGradient (Tones de Status)">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <BackgroundGradient tone="default" containerClassName="w-full">
            <div className="rounded-[22px] bg-white dark:bg-zinc-900 p-4 text-center text-xs font-bold text-slate-800 dark:text-zinc-200">
              🇧🇷 default (V7M)
            </div>
          </BackgroundGradient>

          <BackgroundGradient tone="danger" containerClassName="w-full">
            <div className="rounded-[22px] bg-white dark:bg-zinc-900 p-4 text-center text-xs font-bold text-red-700 dark:text-red-400">
              🔴 danger (Pendente / Recusado)
            </div>
          </BackgroundGradient>

          <BackgroundGradient tone="warning" containerClassName="w-full">
            <div className="rounded-[22px] bg-white dark:bg-zinc-900 p-4 text-center text-xs font-bold text-amber-700 dark:text-amber-400">
              🟡 warning (Em análise / IA)
            </div>
          </BackgroundGradient>

          <BackgroundGradient tone="success" containerClassName="w-full">
            <div className="rounded-[22px] bg-white dark:bg-zinc-900 p-4 text-center text-xs font-bold text-emerald-700 dark:text-emerald-400">
              🟢 success (Aprovado / MEC)
            </div>
          </BackgroundGradient>
        </div>
      </Section>
      ...
      {/* ── media stage card ────────────────────────────────────────── */}
      <Section title="🎴 MediaStageCard (Universal Asset Lifecycle)">
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          <MediaStageCard
            title="1. Vazio / Pendente"
            description="Slot de upload limpo com drag & drop."
            status="empty"
            onUpload={() => {}}
          />

          <MediaStageCard
            title="2. Em Análise (IA)"
            description="Processamento e conferência em andamento."
            status="analyzing"
            onUpload={() => {}}
          />

          <MediaStageCard
            title="3. Aprovado (ChromaticImage)"
            description="Visual rico com aberração cromática no hover."
            status="approved"
            fileUrl="data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='400' height='300' viewBox='0 0 400 300'><rect width='400' height='300' fill='%230b1b3b'/><text x='50%25' y='50%25' dominant-baseline='middle' text-anchor='middle' fill='%2338d178' font-family='sans-serif' font-size='20' font-weight='bold'>CERTIFICADO APROVADO</text></svg>"
            onUpload={() => {}}
            onView={() => {}}
            onRetry={() => {}}
          />
