...
def login_with_profile_uuid_otp(*, profile_uuid, otp):
    ...
    if not profile:
        return ServiceResponse.fail("Perfil não encontrado.")
    ...
    if not otp_code:
        return ServiceResponse.fail("Código de verificação obrigatório.")

    user = profile.user
    if not user.check_password(otp_code):
        return ServiceResponse.fail("Código de verificação inválido.")
