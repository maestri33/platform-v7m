...
## Modelo de roles (no código) — Modelo Assíncrono (2026-08-26)

`lib/auth/roles.ts` separa em três eixos:
- **stage**: `candidate` → `promoter` (onboarding do afiliado). Pós-login o pouso é sempre no `/painel`.
- **gate**: `training` (gate de status pleno / liberação de saques via Pix — NÃO bloqueia o painel, a navegação nem a captação de leads).
- **grant**: `coordinator` (aditivo, empilha sobre `promoter`).

`staff` é reconhecida pra rotear pra superfície global (expansão 2026-06-21).

### Princípios do Modelo Assíncrono
1. Pós-cadastro/login, o afiliado tem acesso imediato ao painel, link de indicação, QR Code e WhatsApp.
2. O preenchimento do dossiê (documento, endereço, Pix, escolaridade, selfie) e treinamento ocorrem de forma assíncrona.
3. As comissões acumulam imediatamente e são pagas no fechamento de **sexta-feira, às 18h** (após aprovação do dossiê).
4. Rolagem vertical natural (`.app-scroll`), sem FitViewport e sem steppers bloqueantes.
