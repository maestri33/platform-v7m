...
App do promotor/afiliado (candidato → capacitação → promotor pleno) no monorepo V7M.

- **Stack:** Next.js 16 (App Router + Turbopack) + React 19 + TypeScript + Tailwind CSS v4 (`@theme inline`).
- **Modelo de Produto:** **Assíncrono pós-cadastro/login**. A captação de leads é liberada no primeiro segundo pós-login no `/painel`. O preenchimento do dossiê (documento, endereço, chave Pix, escolaridade, selfie) e treinamento ocorrem no tempo do afiliado para liberar os saques financeiros.
- **Fechamento Financeiro:** Toda **sexta-feira às 18h** direto na chave Pix validada.
- **Backend consumido:** `/api/v1/collaborators/` (Django 5.2 Ninja).

## Comandos

```bash
pnpm --filter app-promotor dev          # dev server (porta 3001)
pnpm --filter app-promotor run lint     # linting com ESLint v9
pnpm --filter app-promotor run build    # build de produção Next.js Turbopack
pnpm --filter app-promotor check-types  # validação estática TypeScript
```
