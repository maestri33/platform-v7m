 ...
 * OpenAI-compatible). A chave da IA NUNCA vai pro browser — fica aqui, server-side.
 *
 * Precedência de rota (confirmado na doc do Next 16): arquivos de rota vêm ANTES dos rewrites
 * `afterFiles`, então este `/api/copilotkit` é servido pelo Next; o rewrite `/api/:path*` → Django
 * (next.config) só pega o resto. Sem colisão.
 *
...
// não amarra numa versão específica. Generative UI do CopilotKit EXIGE tool_calling.
const MODEL = process.env.OMNIROUTE_MODEL ?? "auto/best-vision";

// OpenAI SDK apontado pro OmniRoute (é OpenAI-compatible: /v1/chat/completions).
