...
import type { CandidateAwaitingRow } from "@/lib/types";
import {
  UserCheck,
  Eye,
  CheckCircle2,
...
export default function CandidatosPage() {
  ...
  const { data: candidateDetail, isLoading: loadingDetail } = useQuery({
    queryKey: ["candidate-detail", selectedCandidate?.external_id],
  ...
  const actionMutation = useMutation({
    ...
    onError: (err: any) => {
      toast.error(err.detail || err.message || "Falha ao processar ação.");
    },
  });
  ...
  const columns: ColumnDef<CandidateAwaitingRow>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-xs text-brand-muted">{formatDateBR(row.original.since)}</span>
      ),
    },
    {
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs bg-brand-bg/50 p-4 rounded-xl border border-brand-border">
                ...
                <div>
                  ...
                  <strong className="text-brand-ink">{formatPhoneBR(candidateDetail?.user?.phone)}</strong>
                </div>
                <div>
