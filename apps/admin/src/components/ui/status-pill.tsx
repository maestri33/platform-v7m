...
/**
 * Mapa status (texto livre do backend) → tom + rótulo amigável. Cobre os status
 * mais comuns de comissões/payouts/matrículas/leads; o que não casar cai em
 * neutral com o texto cru capitalizado.
 */
...
/**
 * Etiqueta de status compacta com cor por significado. Usada em toda tabela
 * (comissões, payouts, matrículas, leads, checks). Status desconhecido → neutral.
 */
export function StatusPill({ status, tone, label }: StatusPillProps) {
  ...
  return (
    <span
      className={`inline-flex items-center whitespace-nowrap rounded-full px-2.5 py-0.5 text-[12px] font-bold ring-1 ring-inset ${CLS[resolvedTone]}`}
    >
      {text}
    </span>
  );
