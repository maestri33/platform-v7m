...
export default function HubMatriculasPage() {
  ...
  const concludeMutation = useMutation({
    mutationFn: async () => {
      ...
      return fetch(`/api/v1/leadership/enrollments/${selectedEnrollment.external_id}/conclude`, {
        method: "POST",
  ...
  const columns: ColumnDef<HubEnrollmentRow>[] = [
    {
      ...
      cell: ({ row }) => (
        <div className="space-y-0.5">
          <div className="font-semibold text-brand-ink">{row.original.student_name || "Sem identificação"}</div>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-xs text-brand-ink font-medium">
          {row.original.promoter_name || "Venda Direta"}
        </span>
      ),
    },
    {
  ...
  return (
    <PageShell
      ...
      description="Acompanhamento e conclusão de matrículas vinculadas exclusivamente ao seu polo regional."
      badge={
    ...
    >
      <div className="space-y-6">
        <DataTableTanstack
          columns={columns}
          data={enrollments || []}
          searchPlaceholder="Filtrar por aluno, promotor ou ID..."
          ...
          <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                ...
                Matrícula: {selectedEnrollment.student_name || "Detalhes"}
              </DialogTitle>
              <DialogDescription>
            ...
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-xs bg-brand-bg/50 p-4 rounded-xl border border-brand-border">
                ...
                <div>
                  <span className="text-brand-muted block">Promotor Vinculado:</span>
                  <strong className="text-brand-ink">{selectedEnrollment.promoter_name || "Venda Direta"}</strong>
                </div>
                <div>
                  <span className="text-brand-muted block">Necessita Revisão:</span>
                  <strong>{selectedEnrollment.needs_review ? "⚠️ Sim" : "✅ Não"}</strong>
                </div>
                <div>
                  <span className="text-brand-muted block">Divergência Cadastral:</span>
                  <strong>{selectedEnrollment.has_divergence ? "⚠️ Detectada" : "✅ Nenhuma"}</strong>
                </div>
              </div>
        ...
        <Dialog open={concludeModalOpen} onOpenChange={setConcludeModalOpen}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Informe o login e senha gerados no sistema acadêmico.
              </DialogDescription>
            </DialogHeader>
