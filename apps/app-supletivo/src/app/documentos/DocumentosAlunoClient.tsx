  ...
  const handleUpload = async (item: DocumentItem, file: File) => {
    try {
      ...
      setItems((prev) =>
        prev.map((doc) => {
          if (doc.id === item.id) {
            let extractedInfo = "Documento acadêmico validado com sucesso";
            if (doc.id === "identity") {
              extractedInfo = "RG / CIN Oficial conferido e aprovado (MEC/SISTEC)";
            } else if (doc.id === "address") {
              extractedInfo = "Comprovante conferido via OCR";
            } else if (doc.id === "school_history") {
              extractedInfo = "Histórico Escolar do Ensino Fundamental registrado";
            } else if (doc.id === "civil_certificate") {
              extractedInfo = "Certidão Civil de Registro verificada";
            } else if (doc.id === "voter_card") {
              extractedInfo = "Quitação Eleitoral TSE validada";
            } else if (doc.id === "military_certificate") {
              extractedInfo = "Certificado de Reservista Militar registrado";
            }
